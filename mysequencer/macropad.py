...
class ArduinoMacropad(Macropad):
    ...
    def set_led(self, pixel_index, r, g, b, brightness):
        self._work_packets.append(Pixel(index=pixel_index, r=r,g=g,b=b,brightness=brightness))

    def _read_func(self):
        incoming_string = ""
        while self._delete is False:
            incoming_string += self.serial_connection.read(self.serial_connection.in_waiting).decode('utf-8')
            new_line_index = incoming_string.find("\n")
    ...
    def _write_func(self):
        while self._delete is False:
            for packet in self._work_packets:
                function_number = "d" if packet.r == 0 and packet.g == 0 and packet.b ==0 else "i"
